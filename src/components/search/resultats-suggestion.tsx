...
import { Search, ExternalLink, ImageIcon, TrendingUp, Tag } from "lucide-react"
...
// Fonction pour générer des recherches associées en fonction de la requête
function getRelatedSearches(query: string): string[] {
  if (!query) return []

  // Recherches associées pour différentes catégories
  const baseSearches: Record<string, string[]> = {
    smartphone: [
      "smartphone pas cher",
      "smartphone haut de gamme",
      "smartphone pliable",
      "smartphone avec bonne batterie",
      "smartphone avec bonne caméra",
      "comparatif smartphones 2023",
    ],
    ordinateur: [
      "ordinateur portable léger",
      "ordinateur de bureau gaming",
      "ordinateur pour graphistes",
      "ordinateur pas cher",
      "meilleur ordinateur 2023",
      "ordinateur tout-en-un",
    ],
    service: [
      "service de réparation smartphone",
      "service de livraison express",
      "service d'installation informatique",
  ...
  if (lowerQuery.includes("smartphone") || lowerQuery.includes("téléphone") || lowerQuery.includes("mobile")) {
    return baseSearches.smartphone
  } else if (lowerQuery.includes("ordinateur") || lowerQuery.includes("pc") || lowerQuery.includes("laptop")) {
    return baseSearches.ordinateur
  } else if (lowerQuery.includes("service") || lowerQuery.includes("réparation") || lowerQuery.includes("livraison")) {
    return baseSearches.service
  }

  // Par défaut, retourner des recherches génériques
  return baseSearches.default
}

// Composant pour afficher les recherches associées
export function RelatedSearches({ query }: { query: string }) {
  ...
  useEffect(() => {
    if (query) {
      ...
      setTimeout(() => {
        setSearches(getRelatedSearches(query))
        setLoading(false)
      }, 300)
    } else {
      setSearches([])
    ...
    return (
      <div className="bg-white rounded-lg shadow-sm p-4">
        ...
        <div className="flex flex-wrap gap-2">
          {[...Array(6)].map((_, i) => (
            <div key={i} className="h-8 w-32 bg-gray-100 animate-pulse rounded-full"></div>
          ))}
        </div>
      </div>
    )
